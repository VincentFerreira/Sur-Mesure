...
const MissingKeywordsCard: React.FC<Props> = ({ rows }) => {
  ...
  return (
    <InsightCard title="Mots-clés à ajouter au CV" testId="insight-card-keywords" fullWidth>
      {rows.length === 0 ? (
        <p className="text-sm text-slate-400" data-testid="insight-empty-keywords">
          Aucun mot-clé manquant sur vos postes scorés — beau travail.
        </p>
      ) : (
          ...
          {rows.map((row) => (
            <div key={row.keyword} data-testid={`missing-keyword-${row.keyword}`}>
              <div className="flex items-center gap-3">
                ...
                {row.isCritical && (
                  <span className="text-xs font-medium px-2 py-0.5 rounded-full bg-amber-50 text-amber-700 shrink-0">Critique</span>
                )}
                ...
                <span className="text-xs text-slate-400 shrink-0">
                  manquant dans {row.missingCount} poste{row.missingCount > 1 ? 's' : ''}
                  {row.partialCount > 0 ? ` · partiel dans ${row.partialCount}` : ''}
                </span>
              </div>
            </div>
          ))}
